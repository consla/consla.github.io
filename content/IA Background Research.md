...
$\hat{\tau}$ is the unit tangent vector of the path at the nearest point, $\vec{e}$ is the error function, $k_{n}\vec{e}$ is the weighted error vector, $\vec{v}$ is the resulting vector from adding $\hat{\tau}$ and $k_{n}\vec{e}$, and $\hat{m_{d}}$ is the unit vector of $\vec{v}$, in other words just the desired direction of motion. It is interesting to note that for any point on the path, the positions of the robot for which that point is the nearest point lies along the direction given by the normal vector $\hat{n}$.
...
Here is an example graph in Desmos:
...
"Nearest point" can be rephrased as "$t$ where distance is minimized", and so we can just find the extrema (via when the derivative is $0$) of the distance formula and select the one that gives the smallest distance. $f(t) = \sqrt{ x(t)^{2} + y(t)^{2} }$ where $x(t)$ and $y(t)$ are cubic polynomials creates sextic polynomials, the derivative of which is a quintic polynomial, whose roots [are not generally solvable](https://en.wikipedia.org/wiki/Quintic_function#Finding_roots_of_a_quintic_equation).
